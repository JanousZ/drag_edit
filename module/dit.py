...
from diffusers.loaders import PeftAdapterMixin, FromOriginalModelMixin, FluxTransformer2DLoadersMixin
...
class FluxTransformer2DPointsModel(
    ...
    def _initialize_custom_layers(self):
        # 检查是否在 meta 设备上
        if self.points_embedder.weight.is_meta:
            # 注意：这里先不要指定具体的 cuda:0，用 .device 自动跟随
            device = torch.device("cpu") # 默认先回 CPU，由 accelerate 随后 dispatch
    ...
    def forward(
        ...
        #hidden_states = torch.cat([encoder_hidden_states, hidden_states], dim=1)  注意diffusers版本问题 这是0.35.2
